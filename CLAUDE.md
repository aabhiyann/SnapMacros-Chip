# Claude Code Instructions — SnapMacros-Chip

## Commits

- **Never** add `Co-Authored-By` trailers or any AI agent attribution to commit messages. The user is the sole author of every commit.
- Do not include Claude, Anthropic, or any AI tool name in commit messages in any form.
...
1. Make one small change with Edit/Write tools
2. Run `npx tsc --noEmit` to verify TypeScript is clean
3. Stage only that specific file: `git add <file>`
4. Commit with a descriptive conventional commit message (no Co-Authored-By)
